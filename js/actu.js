...
function renderArticle(article = {}) {
  ...
  // État initial pour l'animation (slide only, pas d'opacité → pas de flash blanc)
  ...
  requestAnimationFrame(() => {
    cards.forEach((card, i) => {
      setTimeout(() => {
        ...
        // relâche le will-change après l'anim
...
/* ---------- chargement de la liste (loader retardé + fetch parallèle) ---------- */

async function loadNews() {
  const list = document.getElementById('news-list');
  if (!list) return;

  // Nettoie d’anciens états
  list.querySelectorAll('.status').forEach(el => el.remove());

  // Loader retardé (évite le flash)
  ...
  try {
    ...
    // Construis les cartes
    const frag = document.createDocumentFragment();
    data.forEach(item => frag.appendChild(renderArticle(item)));

    // Garde le loader jusqu'au 1er paint, puis anime et retire le loader
